...
def raw_img_to_patches(path_raw_data, path_patched_data, thresh_indices = [0, 0.2, 0.8],
    ...
    #Normally set to 1 (Can be changed to fix the problem that mask images were downsized from myeltracer images)
    orig_mask_scale = 1
    ...
    for img_folder in tqdm(img_folder_names):
        ...
        if path_img_folder.is_dir():
            ...
            pixel_size = pixel_size / orig_mask_scale
            ...
            for data in data_names:
                if 'image' in data: # If it's the raw image.
                    ...
                    img = rescale(img, orig_mask_scale, preserve_range=True, mode='constant').astype(int)
...
def split_data(raw_dir, out_dir, seed=2019, split = [0.8, 0.1, 0.1], override=False):
    """
    Splits a dataset into training, validation, and test folders.
    ...
    :param split: Split fractions ([Train, Validation, Test]).
    ...
    num_train = round(len(sorted_indices)*split_train)
    num_val = round(len(sorted_indices)*split_valid)
    num_test = round(len(sorted_indices)*split_test)

    for index in sorted_indices[:num_train]:
        dirs[index].rename(train_dir / dirs[index].parts[-1])

    for index in sorted_indices[num_train:(num_train+num_val)]:
        dirs[index].rename(valid_dir / dirs[index].parts[-1])

    for index in sorted_indices[-num_test:]:
        dirs[index].rename(test_dir / dirs[index].parts[-1])
